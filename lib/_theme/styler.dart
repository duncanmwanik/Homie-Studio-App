...
class AppColors {
  ...
  static const Color lightPrimary = Color(0xfffffcf8);

  static const Color darkSecondary = Color(0xff272829);
  static const Color lightSecondary = Color(0xfffafafa);
...
class AppStyles {
  ...
  Color appColor([double weight = 1]) {
    return Colors.grey.withOpacity((isLight() ? (weight <= 8 ? weight + 0.6 : weight) : weight) / 10);
  }
  ...
  Color? getItemColor([String bgColor = '']) {
    return bgColor.isNotEmpty
        ? getItemBgColor(bgColor, true).withOpacity(isDark ? 0.1 : 0.5)
        : isImage()
  ...
  Gradient gradient([List<Color>? colors]) {
    return LinearGradient(
      ...
      colors: colors ?? [styler.accentColor(), tertiaryColor()],
    );
  }

  Gradient gradients([List<Color>? colors]) {
    return LinearGradient(
