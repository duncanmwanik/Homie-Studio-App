...
class _SegmentScreenState extends State<SegmentScreen> {
  ...
  void initState() {
    ...
    setState(() {
      segList = prefs.getStringList("seg${widget.segments}") ?? segmentsMap[widget.segments]!;
    });
    sendDataToDevice("g${widget.segments}|${segList.join('|')}|");
  }
                        ...
                        onPressed: () async {
                          await showModalBottomSheet(
                            ...
                            if (value != null) {
                              ...
                              prefs.setStringList("seg${widget.segments}", segList);
                              sendDataToDevice("g${widget.segments}|${segList.join('|')}|");
                            }
                          });
                        },
